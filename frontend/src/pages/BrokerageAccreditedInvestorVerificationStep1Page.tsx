...
import type {
  Baiv506cStepOneFields,
  Baiv506cStepOneQuestionConfig,
  Baiv506cStepOneQuestionId,
  Baiv506cStepOneResponse,
  Baiv506cStepOneUpdateRequest
} from '../types/api';

function createEmptyStep1Fields(): Baiv506cStepOneFields {
  return {
    accountRegistration: {
      rrName: '',
      rrNo: '',
...
const QUESTION_CONFIG: Record<Baiv506cStepOneQuestionId, Baiv506cStepOneQuestionConfig> = {
  'step1.accountRegistration': {
    key: 'step1.accountRegistration',
    title: "Let's quickly confirm the account details.",
    helper: 'We prefilled what we already have so you should only need to review or tweak.',
    type: 'account-registration-block'
  }
};

function findQuestionIndex(
  currentQuestionId: Baiv506cStepOneQuestionId | null,
...
function getAnswer(fields: Baiv506cStepOneFields, questionId: Baiv506cStepOneQuestionId): unknown {
...
function applyAnswer(
  ...
  if (questionId === 'step1.accountRegistration') {
    next.accountRegistration = answer as Baiv506cStepOneFields['accountRegistration'];
  }

  return next;
}

export function BrokerageAccreditedInvestorVerificationStep1Page() {
  ...
  const [fields, setFields] = useState<Baiv506cStepOneFields>(createEmptyStep1Fields());
  const [visibleQuestionIds, setVisibleQuestionIds] = useState<Baiv506cStepOneQuestionId[]>([]);
  const [currentQuestionId, setCurrentQuestionId] = useState<Baiv506cStepOneQuestionId | null>(null);
  ...
  useEffect(() => {
    ...
    const loadStep = async () => {
      ...
      try {
        const response = await apiRequest<Baiv506cStepOneResponse>(
          `/api/clients/${clientId}/brokerage-accredited-investor-verification/step-1`
        );
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    const payload: Baiv506cStepOneUpdateRequest = {
  ...
  const renderActiveControl = () => {
    ...
    const answer = currentAnswer as Baiv506cStepOneFields['accountRegistration'];
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col px-6 pb-10 pt-8 sm:px-12 sm:pt-10">
        ...
        <section className="flex flex-1 flex-col justify-center py-10 sm:py-14">
          <p className="text-xs uppercase tracking-[0.22em] text-accent">
            BROKERAGE ACCREDITED INVESTOR VERIFICATION - STEP 1
          </p>
