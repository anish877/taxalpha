...
import type {
  Baiv506cStepTwoFields,
  Baiv506cStepTwoQuestionConfig,
  Baiv506cStepTwoQuestionId,
  Baiv506cStepTwoResponse,
  Baiv506cStepTwoUpdateRequest
} from '../types/api';

function createEmptyStep2Fields(): Baiv506cStepTwoFields {
  return {
    acknowledgements: {
...
const ACKNOWLEDGEMENT_ITEMS: Array<{
  key: keyof Baiv506cStepTwoFields['acknowledgements'];
  label: string;
}> = [
  {
    key: 'rule506cGuidelineAcknowledged',
    label:
      'I understand this form helps verify accredited status under SEC Rule 506(c) and supports compliance with applicable securities laws.'
  },
  {
    key: 'secRuleReviewedAndUnderstood',
    label:
      'I have reviewed the Rule 506(c) requirements and understand why this verification is required.'
  },
  {
    key: 'incomeOrNetWorthVerified',
    label:
      'I attest that the income or net worth threshold required to qualify as an accredited investor has been met.'
  },
  {
    key: 'documentationReviewed',
    label:
      'I have provided/reviewed acceptable documentation (such as statements, appraisals, credit report, and tax forms) for verification.'
  }
];

const QUESTION_CONFIG: Record<Baiv506cStepTwoQuestionId, Baiv506cStepTwoQuestionConfig> = {
  'step2.acknowledgements': {
    key: 'step2.acknowledgements',
    title: 'Please confirm these four required acknowledgements.',
    helper: 'Each item must be checked before signature capture.',
    type: 'acknowledgements-block'
  },
  'step2.signatures.accountOwners': {
    key: 'step2.signatures.accountOwners',
    title: 'Now capture account owner signatures.',
    helper: 'Typed signature, printed name, and date are required.',
...
function findQuestionIndex(
  currentQuestionId: Baiv506cStepTwoQuestionId | null,
...
function getErrorForQuestion(
  ...
  if (questionId === 'step2.signatures.accountOwners') {
    const key = Object.keys(fieldErrors).find((item) => item.startsWith('step2.signatures.accountOwners.'));
    return key ? fieldErrors[key] : null;
  }

  if (questionId === 'step2.signatures.financialProfessional') {
    const key = Object.keys(fieldErrors).find((item) =>
      item.startsWith('step2.signatures.financialProfessional.')
    );
    return key ? fieldErrors[key] : null;
  }
...
function getAnswer(fields: Baiv506cStepTwoFields, questionId: Baiv506cStepTwoQuestionId): unknown {
  switch (questionId) {
...
function applyAnswer(
  ...
  switch (questionId) {
    case 'step2.acknowledgements':
      next.acknowledgements = answer as Baiv506cStepTwoFields['acknowledgements'];
      break;
    case 'step2.signatures.accountOwners': {
      const payload = answer as {
        accountOwner: Baiv506cStepTwoFields['signatures']['accountOwner'];
        jointAccountOwner: Baiv506cStepTwoFields['signatures']['jointAccountOwner'];
      };
    ...
    case 'step2.signatures.financialProfessional': {
      const payload = answer as {
        financialProfessional: Baiv506cStepTwoFields['signatures']['financialProfessional'];
      };
...
export function BrokerageAccreditedInvestorVerificationStep2Page() {
  ...
  const [fields, setFields] = useState<Baiv506cStepTwoFields>(createEmptyStep2Fields());
  const [visibleQuestionIds, setVisibleQuestionIds] = useState<Baiv506cStepTwoQuestionId[]>([]);
  const [currentQuestionId, setCurrentQuestionId] = useState<Baiv506cStepTwoQuestionId | null>(null);
  ...
  useEffect(() => {
    ...
    const loadStep = async () => {
      ...
      try {
        const response = await apiRequest<Baiv506cStepTwoResponse>(
          `/api/clients/${clientId}/brokerage-accredited-investor-verification/step-2`
        );
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    const payload: Baiv506cStepTwoUpdateRequest = {
  ...
  const renderActiveControl = () => {
    ...
    if (activeQuestion.type === 'acknowledgements-block') {
      const answer = currentAnswer as Baiv506cStepTwoFields['acknowledgements'];
    ...
    return renderSignatureFields(
      ...
      (nextValue) => {
        ...
        setFields((current) => applyAnswer(current, 'step2.signatures.financialProfessional', payload));
      }
    );
  };
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col px-6 pb-10 pt-8 sm:px-12 sm:pt-10">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              onClick={() =>
                navigate(
                  clientId
                    ? `/clients/${clientId}/brokerage-accredited-investor-verification/step-1`
                    : '/dashboard'
                )
              }
            >
        ...
        <section className="flex flex-1 flex-col justify-center py-10 sm:py-14">
          <p className="text-xs uppercase tracking-[0.22em] text-accent">
            BROKERAGE ACCREDITED INVESTOR VERIFICATION - STEP 2
          </p>
