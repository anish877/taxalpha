...
describe('InvestorProfileStep4Page', () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it('loads grouped phones question and submits grouped answer patch', async () => {
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
          ...
          JSON.stringify({
            onboarding: {
              ...
              step: {
                ...
                currentQuestionId: 'step4.holder.contact.phones',
                currentQuestionIndex: 0,
                visibleQuestionIds: ['step4.holder.contact.phones', 'step4.holder.kind'],
                fields: baseStep4Fields
              }
            }
          }),
          {
      ...
      if (url.includes('/api/clients/client_1/investor-profile/step-4') && init?.method === 'POST') {
        return new Response(
          JSON.stringify({
            onboarding: {
              clientId: 'client_1',
              ...
              step: {
                ...
                fields: {
                  ...baseStep4Fields,
                  holder: {
                    ...baseStep4Fields.holder,
                    contact: {
                      ...baseStep4Fields.holder.contact,
                      phones: {
                        home: null,
                        business: null,
    ...
    window.history.pushState({}, '', '/clients/client_1/investor-profile/step-4');
    ...
    await waitFor(() => {
      expect(
        screen.getByText('STEP 4. SECONDARY ACCOUNT HOLDER INFORMATION (Joint Holder #2, Trustee #1, Entity Manager)')
      ).toBeInTheDocument();
      expect(screen.getByText('What are the best phone numbers for this holder?')).toBeInTheDocument();
    });
    ...
    await waitFor(() => {
      expect(fetchMock).toHaveBeenCalledWith(
        expect.stringContaining('/api/clients/client_1/investor-profile/step-4'),
    ...
    const postCall = fetchMock.mock.calls.find(
      (call) =>
        String(call[0]).includes('/api/clients/client_1/investor-profile/step-4') &&
        call[1]?.method === 'POST'
    );
    expect(postCall).toBeDefined();

    const requestBody = JSON.parse(String(postCall?.[1]?.body));
    expect(requestBody.questionId).toBe('step4.holder.contact.phones');
    expect(requestBody.answer.mobile).toBe('+1 555 555 1212');

    await waitFor(() => {
      expect(screen.getByText('Is the secondary account holder a person or an entity?')).toBeInTheDocument();
    });
  });
  ...
  it('submits grouped investment knowledge payload including conditional other fields', async () => {
          ...
          JSON.stringify({
            onboarding: {
              ...
              step: {
                key: 'STEP_4_SECONDARY_ACCOUNT_HOLDER_INFORMATION',
                label: 'STEP 4. SECONDARY ACCOUNT HOLDER INFORMATION (Joint Holder #2, Trustee #1, Entity Manager)',
                currentQuestionId: 'step4.investment.knowledgeExperience',
                currentQuestionIndex: 0,
                visibleQuestionIds: ['step4.investment.knowledgeExperience'],
                fields: baseStep4Fields
              }
            }
          }),
    ...
    await waitFor(() => {
      ...
      expect(body.questionId).toBe('step4.investment.knowledgeExperience');
  ...
  it('navigates to step 5 on final submit', async () => {
    const user = userEvent.setup();
